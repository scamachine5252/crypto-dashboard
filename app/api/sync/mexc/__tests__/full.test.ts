...
function makePost(body: Record<string, unknown>): NextRequest {
  return new NextRequest('http://localhost/api/sync/mexc/full', {
    method: 'POST',
...
// POST
// ---------------------------------------------------------------------------
describe('POST /api/sync/mexc/full', () => {
  beforeEach(() => {
    jest.resetModules()
    jest.clearAllMocks()
  })
    ...
    mockGetTrades.mockResolvedValue([
      {
        ...
        pnlPercent: 2, fee: 5, durationMin: 60, leverage: 1,
        fundingCost: 0, isOvernight: false,
        openedAt: '2025-01-01T00:00:00.000Z',
        closedAt: '2025-01-01T01:00:00.000Z',
        subAccountId: 'mexc', exchangeId: 'mexc',
      },
    ])
    mockUpsert.mockResolvedValue({ error: null })
  ...
  it('upserts raw_fills rows with exchange set to "mexc"', async () => {
    mockSelectEqSingle.mockResolvedValue({
      data: { id: 'uuid-1', api_key: 'enc-key', api_secret: 'enc-sec' },
      error: null,
    })
    mockGetTrades.mockResolvedValue([
      {
        id: 't1', symbol: 'ETH/USDT', side: 'long', tradeType: 'spot',
        entryPrice: 3000, exitPrice: 3100, quantity: 1, pnl: 100,
        pnlPercent: 3.33, fee: 3, durationMin: 30, leverage: 1,
        fundingCost: 0, isOvernight: false,
    ...
    const { POST } = await import('../full/route')
    await POST(makePost({ account_id: 'uuid-1', chunk_index: 0 }))

    // raw_fills rows should have exchange = 'mexc'
    const [rows] = mockUpsert.mock.calls[0]
    expect(rows[0].exchange).toBe('mexc')
  })
  ...
  it('passes correct time window (30 days apart) to getTrades based on chunk_index', async () => {
    mockSelectEqSingle.mockResolvedValue({
      data: { id: 'uuid-1', api_key: 'enc-key', api_secret: 'enc-sec' },
      error: null,
    })
...
// PATCH
// ---------------------------------------------------------------------------
describe('PATCH /api/sync/mexc/full', () => {
  beforeEach(() => {
    jest.resetModules()
    jest.clearAllMocks()
  })
